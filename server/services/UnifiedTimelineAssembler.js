...
class UnifiedTimelineAssembler {
    ...
    buildSynchronizedTimelines(pipelineData, frameCount, metadataSummary) {
        const timelines = {
            ...
            effectTimeline: this.buildEffectTimeline(pipelineData, frameCount, metadataSummary),
            transitionTimeline: this.buildTransitionTimeline(pipelineData, frameCount, metadataSummary)
        };
    ...
    /**
     * Build effect timeline (visual effects, filters, etc.)
     */
    buildEffectTimeline(pipelineData, frameCount, metadataSummary) {
        const timeline = {};
        
        // For now, return empty timeline since we don't have effect detection yet
        // This could be populated from:
        // - Visual effect detection models
        // - Filter recognition
        // - AR effect detection
    ...
    /**
     * Build transition timeline (cuts, fades, wipes, etc.)
     */
    buildTransitionTimeline(pipelineData, frameCount, metadataSummary) {
        const timeline = {};
        
        // Use scene change data to detect hard cuts (most common transition)
        const sceneChanges = pipelineData.scene?.shots || [];
        
        sceneChanges.forEach((shot, index) => {
            if (index > 0) {
                // Mark the start of each new shot as a transition point
                const frame = Math.round(shot.start_time * (metadataSummary.fps || 30));
                const timestamp = this.frameToTimestamp(frame);
                timeline[timestamp] = {
                    frame: frame,
                    type: 'cut',  // Hard cut is the most common transition
                    from_shot: index - 1,
                    to_shot: index,
