...
class EnhancedHumanAnalyzer:
    ...
    def _analyze_scene_segmentation(self, mask, original_image):
        ...
        # Store only essential metrics - no heavy data
        return {
            'person_background_ratio': round(float(person_ratio), 3),
            'background_complexity': round(float(complexity_score), 3),
            'background_type': 'complex' if complexity_score > 0.1 else 'simple',
            'framing_style': self._classify_framing(person_ratio)
        }
    ...
    def _extract_key_landmarks(self, landmarks):
        """Extract key facial landmarks - reduced precision"""
        return {
            'nose_tip': {'x': round(landmarks[1].x, 2), 'y': round(landmarks[1].y, 2)},
            'left_eye': {'x': round(landmarks[33].x, 2), 'y': round(landmarks[33].y, 2)},
            'right_eye': {'x': round(landmarks[263].x, 2), 'y': round(landmarks[263].y, 2)},
            'mouth_center': {'x': round(landmarks[13].x, 2), 'y': round(landmarks[13].y, 2)},
            'left_mouth': {'x': round(landmarks[61].x, 2), 'y': round(landmarks[61].y, 2)},
            'right_mouth': {'x': round(landmarks[291].x, 2), 'y': round(landmarks[291].y, 2)}
        }
    ...
    def _aggregate_gaze_data(self, faces):
        ...
        return {
            ...
            'average_eye_openness': round(np.mean([f['gaze']['eye_openness'] for f in faces]), 2)
        }
    
    def _classify_framing(self, person_ratio):
        """Classify framing style based on person-to-background ratio"""
        if person_ratio > 0.8:
            return 'close_up'
        elif person_ratio > 0.5:
            return 'medium_shot'
        elif person_ratio > 0.2:
            return 'wide_shot'
        else:
            return 'background_focused'
    ...
    def _detect_background_changes(self, frame_analyses):
        """Detect background changes using person ratio and framing analysis"""
        background_changes = []
        change_magnitudes = []
        ...
        for i in range(1, len(sampled_frames)):
            ...
            if 'scene_segmentation' in prev_frame and 'scene_segmentation' in curr_frame:
                prev_ratio = prev_frame['scene_segmentation'].get('person_background_ratio', 0)
                curr_ratio = curr_frame['scene_segmentation'].get('person_background_ratio', 0)
                prev_complexity = prev_frame['scene_segmentation'].get('background_complexity', 0)
                curr_complexity = curr_frame['scene_segmentation'].get('background_complexity', 0)
                
                # Calculate change magnitude based on ratio and complexity changes
                ratio_change = abs(curr_ratio - prev_ratio)
                complexity_change = abs(curr_complexity - prev_complexity)
                change_magnitude = max(ratio_change, complexity_change * 0.5)  # Weight complexity less
                ...
                if change_magnitude > 0.2:  # 20% change threshold
                    ...
                    background_changes.append({
                        ...
                        'magnitude': round(float(change_magnitude), 3),
        ...
        return {
            ...
            'avg_change_magnitude': round(float(np.mean(change_magnitudes)), 3) if change_magnitudes else 0
        }
    ...
    def _generate_summary(self, frame_analyses):
        ...
        # Scene analysis
        person_coverages = [f['scene_segmentation'].get('person_background_ratio', 0) 
                           for f in frame_analyses if 'scene_segmentation' in f]
        if person_coverages:
            summary['scene_analysis']['avg_person_coverage'] = round(np.mean(person_coverages), 3)
