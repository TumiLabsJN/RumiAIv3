...
def compute_visual_overlay_metrics(text_overlay_timeline, sticker_timeline, gesture_timeline, 
    ...
    return {
        ...
        # New fields for complete ML feature support
        'total_sticker_count': total_sticker_count,
        'total_text_elements': total_text_elements,
        'text_categories': text_categories,
        'key_alignment_moments': key_alignment_moments,
        'overall_sync_score': round(overall_sync_score, 2),
        'ml_tags': ml_tags,
        'overlay_strategy': overlay_strategy,
        'pattern_flags': pattern_flags,
        'data_completeness': round(data_completeness, 2),
        'text_detection_rate': round(text_detection_rate, 2),
        'sticker_detection_rate': round(sticker_detection_rate, 2),
        'frame_data_available': True,  # We assume frame data is available
...
def compute_metadata_analysis_metrics(static_metadata, metadata_summary, video_duration):
    ...
    for tag in hashtag_names:
        ...
        if tag_lower in generic_hashtags:
            generic_count += 1
        elif len(tag_lower) > 15:  # Long hashtags are often niche
            niche_count += 1
        else:
            # Could be trending or niche, default to niche
            niche_count += 1
    ...
    # Caption-video alignment placeholders (would need video analysis)
    caption_video_alignment = {
        'topicConsistency': 0.8,  # Placeholder
        'promiseDelivery': 0.85   # Placeholder
    }
    
    # Content category and viral formula (simplified)
    content_category = 'general'  # Would need more analysis
    viral_formula = 'standard'    # Would need pattern matching
    ...
    return {
        ...
        'hashtag_relevance': {
            ...
            'relevance_score': 0.7  # Placeholder - would need content analysis
        },
        'caption_video_alignment': caption_video_alignment,
        ...
        'creator_context': {
            ...
            'follower_count': 0,  # Not available in current data
            'established_creator': False  # Would need follower data
        },
        ...
        'key_phrases': [],  # Would need more NLP
        
        # Patterns
        'content_category': content_category,
        'viral_formula': viral_formula,
        'caption_techniques': [],  # Would need pattern detection
        'linguistic_markers': {
        ...
        'hashtag_patterns': {
            ...
            'sandwich_strategy': False,  # Would need pattern detection
            'all_caps': any(tag.isupper() for tag in hashtag_names),
            'mixed_languages': False  # Would need language detection
        },
        'ml_tags': [],  # Would be generated based on patterns
        
        # Quality
        'data_completeness': 0.8,  # Simplified
...
def extract_real_ml_data(unified_data, prompt_name, video_id=None):
    ...
    if prompt_name == 'visual_overlay_analysis':
        ...
        # Add missing fields expected by prompt
        # speechTimeline is empty since validation is already in computed metrics
        context_data['speechTimeline'] = {}
        
        # Add frame dimensions for mobile optimization calculations
        context_data['frame_dimensions'] = (
            unified_data.get('frame_dimensions') or
            unified_data.get('metadata', {}).get('dimensions') or
            unified_data.get('metadata_summary', {}).get('video_dimensions') or
            {'width': 1080, 'height': 1920}  # Mobile-first default
        )
